...
<div align="center" class="rounded-corners-gray">
  <form name="form1" method="post" onSubmit="EnviarLinkForm('PersInf','<?=$RAIZHTTP?>/modules/client_custom/tse/informes_espdaniel.php?op=1',this);return false;">
    <table width="0" border="0" cellspacing="0" cellpadding="0">
      <tr class="textos_titulos">
        ...
        <td class="textos_titulos">Ponto de venta </td>
        <td class="textos_titulos"><input type="radio" name="camporep" id="radio" value="af13_92"></td>
        <td class="textos_titulos">Campaña Documentos</td>
        <td class="textos_titulos"><input type="radio" name="camporep" id="radio2" value="af13_129"></td>
        <td class="textos_titulos">Proceso</td>
        <td class="textos_titulos"><input type="radio" name="camporep" id="radio3" value="af13_97" /></td>
...
if($_GET[camporep] != ""){ 
	...
	$tablas 		= "autof_matrizprincipal_1,history_1";
	$clausulas 		= "autof_matrizprincipal_1_id = id_reg AND $_GET[intervalo](fechahora)";
	$campos 		= "count($_GET[camporep]) as cuenta";
	$comparalabel	= "$_GET[camporep]";
	$campofecha		= "fechahora";
	$ConsultaLabels = $sqlm->sql_select("autof_".$_GET[camporep],"field1 as LabelC , id_".$_GET[camporep]." as IdComp","1",0);
	$reporteid		= "espdaniel";
...
?> 
 ...
 <!--<a href="<?=$RAIZHTTP?>/modules/client_custom/tse/informes_detail_all.php?qt=<?=$tablas?>&qc=<?=$clausulas?>&qf=<?=$SelectFechas[$o][FechaInt]?>&qfy=<?=$SelectFechas[$o][fecha]?>&qd=<?=$ConsultaLabels[$i][IdComp]?>&rid=<?=$reporteid?>&compc=<?=$comparalabel?>" class="textos <? echo "LinkALL$o"; ?>">-->
 <?=array_sum($ARRdata[$o])?>
 <!--</a>-->&nbsp;</th>
<? } ?>
  </tr> 
