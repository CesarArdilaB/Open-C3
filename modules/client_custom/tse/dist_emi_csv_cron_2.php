...
mail("cac@tse.com.co,andres.ardila@parasuempresa.com","Emision 2 Generadose","Empeso a generarse emision del mes 2"); 


$Date = strtotime(date('Y-m-15'));
$dt_elMesPasado = date('Y-m-',strtotime('-6 month',$Date)) ; // resta 1 mes
...
$SelectDATA = $sqlm->sql_select("autof_matrizprincipal_1","*","DATE(af13_34) BETWEEN '".$dt_elMesPasado."1"."' AND '".$dt_elMesPasado."31"."'",1);

//$SelectDATA = $sqlm->sql_select("autof_matrizprincipal_1,inv_inventario","*","DATE(af13_34) BETWEEN '".$dt_elMesPasado."1"."' AND '".$dt_elMesPasado."31"."' AND idregistro = autof_matrizprincipal_1_id AND idcampana =1 AND (MONTH( fechaentrega ) = '".date("m")."' AND YEAR( fechaentrega ) = '".date("Y")."' OR MONTH( fechasalida ) = '".date("m")."' AND YEAR( fechasalida ) = '".date("Y")."' OR fechasalida = '0000-00-00' OR fechaentrega = '0000-00-00')",1);
...
$htm .= "Mes Fisico|Fecha Fisico TSE|Campaña|Estado Custodia|Identificacion Cliente|SEUDOCODIGO|Nombre Cliente|Activacion|Fecha de Activacion|Fecha de Entrega Envio|Fecha de Entrega|DATOS TERCERO AUTORIZADO|Tipo de Entrega|Nombre Asesor Call Center|Gestion Realizada Call|Fecha de Gestion Call Center|Ciudad Base|Cupo|Nombre Base|Bodega|Gestion Mesa|Punto de Venta|Fecha Entrega|Cliente|Feedback manifiesto|FECHA BANCO BASE AGENDAMIENTO|FECHA RECEPCION BASE TSE|Tipo de entrega inicial|# Bolsa seguridad de salida|Código de dirección|Direccion Cita|TIPO DE EMISION|Fecha Modificacion Agendamiento|Fecha Agendamiento|Mensajero|Agente que Agendo|Id Registro \r";
...
$htm .= $SelectDATA[$i][af13_33]."|".$SelectDATA[$i][af13_34]."|".$campanaTEXT."|".$estadoINV."|".$SelectDATA[$i][af13_39]."|".$SelectDATA[$i][af13_41]."|".$SelectDATA[$i][af13_40]."|".$actData."|".$SelectDATA[$i][af13_128]."|".$fechaSalidaINV."|".$fechaEntregaINV."|".$Indicacion."|".$entData."|".$operadorText."|".$gescallText."|".substr($SelectDATA[$i][af13_34],0,10)."|".$CiudadBText."|".$SelectDATA[$i][af13_42]."|".$SelectDATA[$i][af13_171]."|".$BodegaTxt."|".$gmesaText."|".$pventaText."|".$SelectDATA[$i][af13_93]."|".$FeedBACKT."|".$SelectDATA[$i][af13_35]."|".$SelectDATA[$i][af13_36]."|".$tentregaText."|".$SelectDATA[$i][af13_152]."|".$coddirText."|".$SelectDATA[$i][af13_145]."|".$SelectDATA[$i][af13_52]."|$fechaAG|$fechaN|$MensajeroDATAT|$OperadorAGDATAT|".$SelectDATA[$i][autof_matrizprincipal_1_id]."\r";
...
$new_report=fopen("../../../tmp/emision_distribucion_autogenerado_dia_p2.csv","w");
 
fwrite($new_report, $htm);
 
fclose($new_report);
 
 mail("cac@tse.com.co,andres.ardila@parasuempresa.com","Emision 2 Generadose","Termino de generarse emision del mes 2"); 
