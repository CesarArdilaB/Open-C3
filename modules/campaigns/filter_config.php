...
<div class="textosbig" align="center">
  <h3>Administracion De Filtros</h3>
</div>
<table border="0" cellspacing="2" cellpadding="0" align="center">
  <tr>
	...
	"campoid"=>"id_campaign",
	"condiorden"=>"1",
	"direccion"=>"modules/campaigns/filter_config.php?op=1");
...
<script>
EnviarLinkJ('MuestraReportes','modules/campaigns/filter_config.php?op=2&idcampaign=<?=$_POST[varid]?>');
</script>

<form onSubmit="EnviarLinkForm('MuestraReportes','<?=$RAIZHTTP?>/modules/campaigns/filter_config.php?op=2',this);this.reset();return false;">
          Nuevo Filtro:<br />
  <?=$formulario->c_text("","Nombre_Filtro","","","",":required","",10);?><br>
  <? $parametrosGrupoHerr=array(
	...
	echo $formulario->c_select("","id_form","","",":required",$parametrosGrupoHerr,0,"","MuestraForms"); ?>
...
$guardarForm = $sqlm->inser_data("filter_config","nombre,idform,idcam","'$_GET[Nombre_Filtro]','$_GET[id_form]','$_GET[idcampaign]'",0);
									}
	//-----------------------									
$TraerFiltros = $sqlm->sql_select("filter_config","*","idcam = '$_GET[idcampaign]'",0);
?>
<div class="demo">
<ol id="selectable">
...
<li class="ui-widget-content" onmousedown="EnviarLinkJ('ConfigReport','modules/campaigns/filter_conditions.php?idform=<?=$TraerFiltros[$i][idform]?>&idfiltro=<?=$TraerFiltros[$i][id_filter]?>','este')"><?=$TraerFiltros[$i][nombre]?> <?=$dialers?> | <?=$FormName[0][labeltabla]?> | <input class="BotonesBorrar" type="button" value="Eliminar" onclick="EnviarLinkJ('ConfigReport','modules/campaigns/filter_delete.php?idform=<?=$TraerFiltros[$i][idform]?>&idfiltro=<?=$TraerFiltros[$i][id_filter]?>','este')"  style="position:relative; margin-top:-3px;float: right" /></li>
