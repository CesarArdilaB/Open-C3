...
class UploadHandler extends Services_JSON
{
    private $options;
	var $rutav;
    ...
    private function get_file_object($file_name, $subdir='') {
        $suburl = str_replace('%2F', '/', rawurlencode($this->get_subdir()));
        $file_path = $this->options['upload_dir'].$this->get_subdir().$file_name;
        if (is_file($file_path) && $file_name[0] !== '.') {
            ...
            foreach($this->options['image_versions'] as $version => $options) {
                if (is_file($options['upload_dir'].$this->get_subdir().$file_name)) {
                    $file->{$version.'_url'} = $options['upload_url']
                        .$this->get_subdir().rawurlencode($file->name);
                }
            }
            $file->delete_url = $this->options['script_url']
                .'?file='.rawurlencode($file->name)
                .'&subdir='.rawurlencode($this->get_subdir());
    ...
    private function get_file_objects($list=array(), $subdir='') {
        $upload_dir = $this->options['upload_dir'].$this->get_subdir();
        foreach (scandir($upload_dir) as $file_name) {
            if ($file_name[0] !== '.') {
                $file_path = $upload_dir.$file_name;
                if (is_file($file_path)) {
                    array_push($list, $this->get_file_object($file_name, $this->get_subdir()));
                } elseif (is_dir($file_path)) {
                    $list = $this->get_file_objects($list, $this->get_subdir().$file_name.'/');
                }
            }
        }
        return $list;
    ...
    public function delete() {
        ...
        header('Content-type: application/json');
        echo parent::encode($success);
    }
}

$upload_handler = new UploadHandler();

@$cradirsC=mkdir("../../../files/cam_".$_GET[Idcamania]);
@$cradirsR=mkdir("../../../files/cam_".$_GET[Idcamania]."/".$_GET[IdIdent]); 

$ruta = "cam_".$_GET[Idcamania]."/".$_GET[IdIdent]."/";

$upload_handler->rutav = $ruta;
