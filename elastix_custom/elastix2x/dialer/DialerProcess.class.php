...
WHERE id_campaign = ?
    AND (status IS NULL 
        OR status NOT IN ("Success", "Placing", "Ringing", "OnQueue", "OnHold","Contacted")) 
    AND retries < ?
    AND dnc = 0
...
PETICION_LLAMADAS;
        ...
        if (count($listaLlamadas) > 0) {
            ...
            foreach ($listaLlamadas as $sKey => $tupla) {
                ...
                if ($resultado['Response'] == 'Success') { //ojo aqui Andres.
                    // Guardar el momento en que se originó la llamada
                    $listaLlamadas[$sKey]->OriginateStart = time();
                    $listaLlamadas[$sKey]->OriginateEnd = NULL;
...
"UPDATE `calls` SET status = 'Contacted' WHERE idopen = (SELECT id_open FROM calls_open WHERE id_call = ".$this->_infoLlamadas['llamadas'][$sKey]->id." LIMIT 1) AND id != ".$this->_infoLlamadas['llamadas'][$sKey]->id.""
);
	//Andres Aqui Nuestro Update
...
LEER_RESUMEN_CAMPANIA;
        ...
        $tupla['status'] = array(
			...
			'Contacted' =>  0,  // Este Cliente ya fue Contactado y no se deben marcar los demas registros.          
        );
