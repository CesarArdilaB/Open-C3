...
if($tipoCampo == "S" or $tipoCampo == "SL"){	   
		   ...
		   if($valor == 0){
		$itemRegresar = "<option value='' id='' selected=\"selected\">Seleccione</option>";	   
			}else{
...
mysql_free_result($req);
	...
	function c_text($etiqueta,$nombre,$estilo,$etiqueta_estilo,$valor,$requerido=0,$noedit=0,$ancho=0){
...
return "<label id='$nombre'> $etiquetaPrint <input type='text' name=\"".$nombre."\" value=\"".utf8_encode($valor)."\" id=\"".$estilo."\" $valclase $valencho></label>";
...
return "<input name=\"".$nombre."\" type=\"checkbox\" id=\"".$estilo."\" value=\"".$valor."\" $cheKED /> $etiquetaPrint";
	...
	<select name=\"".$nombre."\" class=\"".$requerido."\""; 
	...
	$regresar = $Valuer[texto];
	
	 }
	
	if($PasarDatoID != 0){$regresar .= " $TextoDatoID";}
	...
	function c_fecha_input($etiqueta,$nombre,$estilo,$etiqueta_estilo,$valor,$requerido,$noedit=0){
